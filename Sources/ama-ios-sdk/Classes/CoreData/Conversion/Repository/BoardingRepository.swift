...
protocol BoardingProtocol: UpdatesProtocol {}

struct BoardingRepository: BoardingProtocol {
    
    typealias T = BoardingMetaDataModel
    let defaults = UserDefaults.standard
    let defaultKey = "Boarding"
    
    func createMethod(record: BoardingMetaDataModel) {
    ...
    func getAllData() -> [BoardingMetaDataModel]? {
        if let data = defaults.data(forKey: defaultKey) {
            // Decode the Data object back to an array of Person objects.
            let decoder = JSONDecoder()
            if let results = try? decoder.decode([BoardingMetaDataModel].self, from: data) {
    ...
    func getById(id: String) -> BoardingMetaDataModel? {
        return nil
    }
    
    func update(record: BoardingMetaDataModel) -> Bool {
        return true
    }
