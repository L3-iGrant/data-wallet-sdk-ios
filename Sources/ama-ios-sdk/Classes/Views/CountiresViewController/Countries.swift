...
open class Countries: Codable {
    ...
    public fileprivate(set) static var countries: [Country] = {
        ...
        if let path = Constants.bundle.path(forResource: "countries_list", ofType: "json") {
            do {
                let data = try Data(contentsOf: URL(fileURLWithPath: path), options: .mappedIfSafe)
                countries =  try! JSONDecoder().decode(CountryListModel.self, from: data)
            } catch {
...
//    }
    ...
    open class func countryFromCountryCode(_ countryCode: String) -> Country {
        for country in countries {
            if countryCode == country.regionLabel {
                return country
            }
        }
        return Country.emptyCountry
    ...
    ///Find country flag by using the unicode of the coutnry
