...
struct PresentationRequestWalletRecordModel: Codable {
    var threadID, createdAt, updatedAt, connectionID, QR_ID: String?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case verified, trace, QR_ID, dataAgreement
    }
...
struct PRRequestedProof: Codable {
    ...
    let selfAttestedAttrs: [String: String]?
    let unrevealedAttrs, predicates: PRRequestedPredicates?
    let revealedAttrGroups: [String:RevealedAttrGroups?]?
...
// MARK: - RevealedAttrGroups
class RevealedAttrGroups:Codable {
    ...
    init(subProofIndex: Int?, values: [String:val]?) {
        self.subProofIndex = subProofIndex
        self.values = values
    }
}
...
class val: Codable {
    ...
    init(raw: String?, encoded: String?) {
        self.raw = raw
...
//        self.subProofIndex = try container.decodeIfPresent(Int.self, forKey: .subProofIndex)
//        self.raw = CodableUtils.decodeAsString(container: container, codingKey: .raw)
