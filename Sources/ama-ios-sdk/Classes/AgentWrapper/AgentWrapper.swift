...
class AgentWrapper {
    ...
    func deleteWallet(withConfig: String, credentials: String, completion: @escaping (Error?) -> Void) {
        wallet?.delete(withConfig: withConfig, credentials: credentials, completion: completion)
    }
    ...
    func pool_prover_get_credentials(forFilter: String!, walletHandle: IndyHandle, completion: @escaping ((Error?, String?) -> Void)){
        IndyAnoncreds.proverGetCredentials(forFilter: forFilter, walletHandle: walletHandle, completion: completion)
    }
    ...
    @available(*, renamed: "signMessage(message:key:walletHandle:)")
    func signMessage(message: Data!, key: String, walletHandle: IndyHandle, completion: @escaping((Error?, Data?) -> Void)) {
        IndyCrypto.signMessage(message, key: key, walletHandle: walletHandle, completion: completion)
    }
    
    func signMessage(message: Data!, key: String, walletHandle: IndyHandle) async -> (Error?, Data?) {
        return await withCheckedContinuation { continuation in
            signMessage(message: message, key: key, walletHandle: walletHandle) { result1, result2 in
                continuation.resume(returning: (result1, result2))
            }
        }
    }
    
    @available(*, renamed: "verifySignature(signature:message:key:walletHandle:)")
    func verifySignature(signature: Data!,message: Data!, key: String, walletHandle: IndyHandle, completion: @escaping((Error?, Bool) -> Void)) {
        IndyCrypto.verifySignature(signature, forMessage: message, key: key, completion: completion)
    }
    
    func verifySignature(signature: Data!,message: Data!, key: String, walletHandle: IndyHandle) async -> (Error?, Bool) {
        return await withCheckedContinuation { continuation in
            verifySignature(signature: signature, message: message, key: key, walletHandle: walletHandle) { result1, result2 in
                continuation.resume(returning: (result1, result2))
            }
        }
    }
    ...
    func getCurrentDateTime(format: String = "yyyy-MM-dd HH:mm:ss.SSSSSS'Z'") -> String {
        let date = Date()
    ...
    func importWallet(withConfig: String,credentials: String, importConfigJson: String,completion: @escaping ((Error?) -> Void)){
        IndyWallet.sharedInstance().import(withConfig: withConfig, credentials: credentials, importConfigJson: importConfigJson, completion: completion)
    }
    
    func exportWallet(withHandle: IndyHandle, exportConfigJson: String,completion: @escaping ((Error?) -> Void)){
        IndyWallet.sharedInstance().export(withHandle: withHandle, exportConfigJson: exportConfigJson, completion: completion)
    }
}
