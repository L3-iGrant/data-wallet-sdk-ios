...
enum CertificatePreviewVC_Mode{
    case EBSI_V2
...
final class CertificatePreviewViewController: AriesBaseViewController, NavigationHandlerProtocol, CustomNavigationBarIconViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        case .EBSI_PDA1:
            ...
            self.companyName.text = self.viewModel?.certModel?.value?.attributes?["name"]?.value ?? ""
            self.tableView.reloadData()
        case .PhotoIDWithAgeBadge:
            self.certificateNameBaseView.isHidden = true
...
extension CertificatePreviewViewController: ValuesRowImageTableViewCellDelegate {
