...
class CertificateListViewController: AriesBaseViewController {
    ...
    let orgTableView = UITableView.getTableview()
//
...
//    var cellHeights = [IndexPath: CGFloat]()
    ...
    override func loadView() {
        super.loadView()
        view.tupleViews(views: orgTableView)
        orgTableView.addAnchorFull(view)
        orgTableView.register(cellType: CertificateWithDataTableViewCell.self)
        orgTableView.register(cellType: RemoveBtnTableViewCell.self)
    ...
    @objc func fetchData() {
        viewModel?.fetchCertificates(completion: { [weak self] (success) in
...
//        } else
        if indexPath.section == 0 {
            ...
            messageLabel.text = "No data available".localizedForSDK()
        ...
        let cell:CertificateWithDataTableViewCell? = tableView.dequeueReusableCell(withIdentifier: "CertificateWithDataTableViewCell") as? CertificateWithDataTableViewCell
            ...
            let attrArray = item?.certificates?.value?.offerCredential?.value?.credentialProposalDict?.credentialProposal?.attributes?.map({ (item) -> IDCardAttributes in
...
//        return cell
            ...
            let cell:RemoveBtnTableViewCell? = tableView.dequeueReusableCell(with: RemoveBtnTableViewCell.self, for: indexPath)
            cell?.selectionStyle = .none
//            if indexPath.row == (tableView.numberOfRows(inSection: indexPath.section) - 1) {
//                cell?.separatorInset = UIEdgeInsets(top: 0, left: cell?.bounds.size.width ?? 0, bottom: 0, right: 0)
//            }else{
//                cell?.separatorInset = UIEdgeInsets.zero
//            }
            cell?.lbl.text = "Remove Organisation".localizedForSDK().uppercaseFirstWords
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 1 { //delete
            let alert = UIAlertController(title: "Data Wallet", message: "Do you want to remove the organisation?".localizedForSDK(), preferredStyle: .alert)
    ...
    @objc func clickedOnAddCert(sender: UIButton){
        ...
        if let controller = UIStoryboard(name:"ama-ios-sdk", bundle:UIApplicationUtils.shared.getResourcesBundle()).instantiateViewController( withIdentifier: "CertificatePreviewViewController") as? CertificatePreviewViewController {
            controller.viewModel = CertificatePreviewViewModel.init(walletHandle: viewModel?.walletHandle, reqId: viewModel?.reqId ?? "", certDetail: cert?.certificates?.value?.offerCredential, inboxId: cert?.certificates?.id,dataAgreement: cert?.certificates?.value?.dataAgreement)
...
extension CertificateListViewController: CustomNavigationBarIconViewDelegate, OrganizationHeaderDelegate{
