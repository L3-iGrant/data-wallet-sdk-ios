...
class AboutViewController: AriesBaseViewController {
    ...
    override func localizableValues() {
        let appVersionString = Constants.bundle.infoDictionary?["CFBundleShortVersionString"] as? String ?? ""
        self.appVersion.text = "Version".localizedForSDK() + ": " + appVersionString
        self.title = "About".localizedForSDK()
    }
    
    @IBAction func tappedOnAriesProtocolsButton(_ sender: Any) {
        if let webviewVC = UIStoryboard(name:"ama-ios-sdk", bundle: Bundle.module).instantiateViewController( withIdentifier: "WebViewVC") as? WebViewViewController {
            webviewVC.urlString = "https://datawallet.igrant.io/aries-supported-protocols.html"
            webviewVC.title = "Supported Protocols".localizedForSDK()
    ...
    @IBAction func tappedOnTermsAndConditionButton(_ sender: Any) {
        if let webviewVC = UIStoryboard(name:"ama-ios-sdk", bundle:Bundle.module).instantiateViewController( withIdentifier: "WebViewVC") as? WebViewViewController {
            ...
            webviewVC.title = "Terms And Conditions".localizedForSDK()
            self.navigationController?.pushViewController(webviewVC, animated: true)
        }
    }
    ...
    @IBAction func tappedOnPrivacyPolicyButton(_ sender: Any) {
        if let webviewVC = UIStoryboard(name:"ama-ios-sdk", bundle:UIApplicationUtils.shared.getResourcesBundle()).instantiateViewController( withIdentifier: "WebViewVC") as? WebViewViewController {
            ...
            webviewVC.title = "Privacy Policy".localizedForSDK()
            self.navigationController?.pushViewController(webviewVC, animated: true)
        }
    }
    /*
