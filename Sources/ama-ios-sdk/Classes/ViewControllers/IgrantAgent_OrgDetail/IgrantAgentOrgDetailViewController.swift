...
class IgrantAgentOrgDetailViewController: AriesBaseViewController {
    ...
    @objc func fetchData() {
        viewModel?.fetchCertificates(completion: { [weak self] (success) in
            self?.orgTableView.isHidden = false
            self?.orgTableView.alpha = 0
            self?.orgTableView.reloadInMain()
            DispatchQueue.main.asyncAfter(deadline: .now() + 1, execute: {
                self?.orgTableView.reloadInMain()
                UIApplicationUtils.hideLoader()
                self?.orgTableView.alpha = 1
            })
        })
    }
...
extension IgrantAgentOrgDetailViewController: UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if  indexPath.section == 0{
            if indexPath.row == 0{
                ...
                UIApplicationUtils.shared.setRemoteImageOn(orgCell.orgImageView, url: viewModel?.orgInfo?.coverImageURL ?? (viewModel?.connectionModel?.value?.orgDetails?.coverImageURL ?? ""),placeholderImage: "00_Default_CoverImage_02-min".getImage())
                return orgCell
            }else{
            ...
            let attrArray = item?.attrArray?.map({ (item) -> IDCardAttributes in
            ...
            cell?.name.text = "Remove Organisation".localizedForSDK().uppercaseFirstWords
            return cell ?? UITableViewCell()
        }
    }
    ...
    @objc func clickedOnAddCert(sender: UIButton){
        ...
        if let controller = UIStoryboard(name:"ama-ios-sdk", bundle:UIApplicationUtils.shared.getResourcesBundle()).instantiateViewController( withIdentifier: "CertificatePreviewViewController") as? CertificatePreviewViewController {
            controller.viewModel = CertificatePreviewViewModel.init(walletHandle: viewModel?.walletHandle, reqId: viewModel?.reqId ?? "", certDetail: cert?.certificates?.value?.offerCredential, inboxId: cert?.certificates?.id, connectionModel: viewModel?.connectionModel,dataAgreement: cert?.certificates?.value?.dataAgreement)
...
extension IgrantAgentOrgDetailViewController: OrganizationDelegate {
